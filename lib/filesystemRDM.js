...
const {observationsDirPath} = require("./filesystemDM");
			...
			.then(
				entries => entries
					...
					.map( respId => {
						...
						return new RDMResponse(respBasePath, respId, this.promise);
					})
			);
		respListPromise
...
function RDMResponse(basePath, respId, parentPromise) {
	this.basePath = basePath;
	this.promise = parentPromise;
	this.id = respId;
	this.deferred = new Deferred();
