...
RDMResponse.prototype = {
	...
	getMeta: function() {
		if (this.isStopped) throw Error();
		if (this.metaPromise !== null) return this.metaPromise;
		this.metaPromise = this.prepareReadDataObjPromise("meta");
		this.childPromises.push(this.metaPromise);
	...
	getHttp: function() {
		if (this.isStopped) throw Error();
		let httpPromise = this.getMeta()
			.then( metaJSON => metaJSON.http )
			.then(
				httpJSON => this.getSecurityInfoDataObj(httpJSON.securityInfo)
					.then(
						siDataObj => cloneReplace(httpJSON, [], {securityInfo: siDataObj})
					)
			);
		this.childPromises.push(httpPromise); // because of possible reading securityInfoRaw
		return httpPromise;
	},
