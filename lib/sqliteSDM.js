...
SDMResponse.prototype = {
	...
	prepareAndSaveHttp: function(http, topic) {
		this.saveHttp(SDMResponse.prepareHttp(http, topic));
	},
	...
	prepareAndSaveHttpStatus: function(httpStatus, dataError) {
		this.saveHttpStatus(SDMResponse.prepareHttpStatus(httpStatus, dataError));
	},
	...
	prepareAndSaveCacheEntry: function(aEntry) {
		...
		if (aEntry === null) {
			...
			outCacheEntry = SDMResponse.prepareCacheEntry(aEntry);
		}
...
SDMResponse.prepareHttp = function(http, topic) {
	var httpReqHeads = [];
	...
	var httpDataObj = {
		...
		securityInfo: (http.securityInfo !== null ? SDMResponse.prepareHttpSecurityInfo(http.securityInfo) : null),
	};
	
	return httpDataObj;
};
SDMResponse.prepareHttpSecurityInfo = function(securityInfo) {
	try {
...
SDMResponse.prepareHttpStatus = function(tracingErr, httpStatus) {
	let tracingStatus;
	{
		let e = tracingErr;
		if (typeof e === "number") {
...
SDMResponse.prepareCacheEntry = function(aEntry) {
	var out = {
		key:               aEntry.key,
	...
	out["meta"] = SDMResponse.prepareCacheMeta(aEntry);
	return out;
};
SDMResponse.prepareCacheMeta = function(aEntry) {
	var out = {};
	var otherMeta = [];
	var reqHeaders = [];
	aEntry.visitMetaData({
		onMetaDataElement: function(key, value) {
			switch(key) {
				...
				case "response-head": {
					try {
						out["response"] = SDMResponse.parseCachedResponseHead(value);
					} catch(e) {
						console.error(e);
					}
					break;
...
const httpHeaderRe = /^([^:]+): (.*)$/;
SDMResponse.parseCachedResponseHead = function(headStr) {
