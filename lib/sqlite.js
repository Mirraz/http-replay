  ...
  _bindParameters: function (statement, params) {
    ...
    if (Array.isArray(params)) {
        ...
        for (let p of params) {
          ...
          for (let [key, value] of Iterator(p)) {
            if (Array.isArray(value)) {
              bindings.bindBlobByName(key, value, value.length);
            } else {
      ...
      for (let i = 0; i < params.length; i++) {
        let value = params[i];
        if (Array.isArray(value)) {
          statement.bindBlobByIndex(i, value, value.length);
        } else {
    ...
    if (params && typeof(params) == "object") {
      for (let k in params) {
        let value = params[k];
        if (Array.isArray(value)) {
          statement.bindBlobByName(k, value, value.length);
        } else {
          statement.bindByName(k, value);
        }
      }
      return;
    }
