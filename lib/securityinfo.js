...
SecurityInfoUtils.securityInfoGetCerts = function(securityInfo) {
	let certs = [];
	if (securityInfo.SSLStatus !== null) certs.push(securityInfo.SSLStatus.serverCert.cert);
	if (securityInfo.failedCertChain !== null) {
		securityInfo.failedCertChain.certList.forEach( certObj => {
...
SecurityInfoUtils.securityInfoCloneReplaceCerts = function(securityInfo, certReplaceCallback) {
	...
	if (SSLStatus === null) {
		...
		let certObjClone = cloneReplace(certObj, [], {cert: certReplaceCallback(certObj.cert)});
	...
	if (failedCertChain === null) {
		...
		certListClone = certList.map(
			certObj => cloneReplace(certObj, [], {cert: certReplaceCallback(certObj.cert)})
		);
