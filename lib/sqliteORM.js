...
{
	table01: {
		col01: function(execute, addSideExecution) {
			addSideExecution(execute({...}));
			return execute({...});
		}
	}
}
...
function executeOrmObj(dbConn, preset, obj) {
	...
	if (!(table in preset)) throw Error("Not preset for table '" + table + "'");
	let tablePreset = preset[table];
	...
	if (tablePresetType === "insert") {
		...
		if (paramsType === "object") {
			...
			if (Object.keys(childs).length === 0) {
				return insertAndGetId(dbConn, tablePresetValue, primitives);
			} else {
				let names = Object.keys(childs);
				return Promise.all(
					names.map( name => executeOrmObj(dbConn, preset, childs[name]) )
				)
				.then( results => {
					...
					return insertAndGetId(dbConn, tablePresetValue, resParams);
				});
			}
		} else { // TODO
