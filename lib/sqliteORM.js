...
function makeTablePresetValue(type, configValue) {
	if (type === "insert") {
		...
		return makeEnumPreset(table, idColName, valueColName);
	} else {
...
function makeTablePreset(type, configValue) {
	let tablePreset = {};
	tablePreset[type] = makeTablePresetValue(type, configValue);
	return tablePreset;
}
