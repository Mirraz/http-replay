...
function SDMCerts(basePath, parentPromise) {
	...
	let dirPromise = parentPromise
		.then( () => OS.File.makeDir(this.basePath) );
	dirPromise
		.catch( e => {console.error(e)} );
	
	let hashSet = new Set();
	this.hashSetFilledPromise = dirPromise
		.then( () => {
	...
	this.filesCache = new FilesCache(this.basePath, dirPromise, 256);
...
SDMCerts.prototype = {
	...
	saveCert: function(certByteArray) {
		...
		let savePromise = this.save(hashPromise, certByteArray);
	...
	// data -- ByteArray
	// private
	save: function(hashPromise, data) {
			...
			.then( results => {
				...
				if (this.hashSet.has(hash)) {
					...
					return this.filesCache.get(hash)
						.then( cachedData => {
							if (! arrayEquals(cachedData, data)) console.error("Collision!!!");
						});
				} else {
...
FilesCache.prototype = {
	...
	add: function(hash, data) {
		...
		let writePromise = this.promise
			.then( () => OS.File.writeAtomic(filePath, new Uint8Array(data), {noOverwrite: true, flush: true}) );
		writePromise
			.catch( e => {console.error(e)} );
		return writePromise;
	},
