...
const {Sqlite} = require("./sqlite"); // Cu.import("resource://gre/modules/Sqlite.jsm");
...
DMResponse.CertDataInsertSql = makeInsertSql("certDatas", ["cert"]);
DMResponse.CertObjInsertSql = makeInsertSql("certObjs", ["cachedEVStatus", "certData_id"]);
DMResponse.SSLStatusInsertSql = makeInsertSql(
	"SSLStatuses",
	[
		"serverCert_id",
		"cipherSuite",
...
DMResponse.SecurityInfoInsertSql = makeInsertSql(
	"securityInfos",
	[
		"securityState",
...
DMResponse.SecurityInfoDataInsertSql = makeInsertSql(
	"securityInfoDatas",
	[
		"securityInfo_id",
		"securityInfoRaw_id"
...
DMResponse.SecurityInfoRawInsertSql = makeInsertSql("securityInfoRaws", ["data"]);
...
DMResponse.prototype = {
	...
	// returns: Promise<certObj_id>
	saveCertObj: function(certObj) {
		return Promise.all([
			this.dbConnPromise,
			insertAndGetId(this.dbConnPromise, DMResponse.CertDataInsertSql, {cert: certObj.cert})
		])
		.then(
			results => insertAndGetIdWithConn(
				results[0],
				DMResponse.CertObjInsertSql,
				{
					cachedEVStatus: certObj.cachedEVStatus,
					certData_id:    results[1]
				}
			)
		);
	},
	
	// returns: Promise<SSLStatus_id>
	saveSSLStatus: function(SSLStatusObj) {
		if (SSLStatusObj === null) return Promise.resolve(null);
		return Promise.all([
			this.dbConnPromise,
			this.saveCertObj(SSLStatusObj.serverCert)
		])
		.then(
			results => insertAndGetIdWithConn(
				results[0],
				DMResponse.SSLStatusInsertSql,
				{
					serverCert_id:              results[1],
					cipherSuite:                SSLStatusObj.cipherSuite,
					protocolVersion:            SSLStatusObj.protocolVersion,
					isDomainMismatch:           SSLStatusObj.isDomainMismatch,
					isNotValidAtThisTime:       SSLStatusObj.isNotValidAtThisTime,
					isUntrusted:                SSLStatusObj.isUntrusted,
					isEV:                       SSLStatusObj.isEV,
					hasIsEVStatus:              SSLStatusObj.hasIsEVStatus,
					haveCipherSuiteAndProtocol: SSLStatusObj.haveCipherSuiteAndProtocol,
					haveCertErrorBits:          SSLStatusObj.haveCertErrorBits,
				}
			)
		);
	},
	
	// returns: Promise<certList_id>
	saveCertList: function(certListObj) {
		if (certListObj === null) return Promise.resolve(null);
		// TODO
		return Promise.resolve(null);
	},
	
	// returns: Promise<securityInfo_id>
	saveSecurityInfo: function(securityInfoObj) {
		if (securityInfoObj === null) return Promise.resolve(null);
		return Promise.all([
			this.dbConnPromise,
			this.saveSSLStatus(securityInfoObj.SSLStatus),
			this.saveCertList(securityInfoObj.failedCertChain),
		])
		.then(
			results => insertAndGetIdWithConn(
				results[0],
				DMResponse.SecurityInfoInsertSql,
				{
					securityState:             securityInfoObj.securityState,
					subRequestsBrokenSecurity: securityInfoObj.subRequestsBrokenSecurity,
					subRequestsNoSecurity:     securityInfoObj.subRequestsNoSecurity,
					errorCode:                 securityInfoObj.errorCode,
					errorMessageCached:        securityInfoObj.errorMessageCached,
					SSLStatus_id:              results[1],
					failedCertChain_id:        results[2],
				}
			)
		);
	},
	
	// returns: Promise<securityInfoData_id>
	saveSecurityInfoData: function(siData) {
		if (siData === null) return Promise.resolve(null);
		if ("parsed" in siData) {
			return Promise.all([
				this.dbConnPromise,
				this.saveSecurityInfo(siData.parsed)
			])
			.then(
				results => insertAndGetIdWithConn(
					results[0],
					DMResponse.SecurityInfoDataInsertSql,
					{securityInfo_id: results[1], securityInfoRaw_id: null}
				)
			);
		} else if ("raw" in siData) {
			return Promise.all([
				this.dbConnPromise,
				insertAndGetId(this.dbConnPromise, DMResponse.SecurityInfoRawInsertSql, {data: siData.raw})
			])
			.then(
				results => insertAndGetIdWithConn(
					results[0],
					DMResponse.SecurityInfoDataInsertSql,
					{securityInfo_id: null, securityInfoRaw_id: results[1]}
				)
			);
		} else {
			throw Error("Empty siData");
		}
	},
	...
	saveHttp: function(obj) {
		var httpChannelIdPromise =
			Promise.all([
				...
				this.saveSecurityInfoData(obj.securityInfo),
			])
			.then( resArr => {
				return {
					dbConn:                           resArr[0],
			...
			.then(
				results => insertAndGetIdWithConn(results.dbConn, DMResponse.HttpChannelInsertSql, {
					...
					securityInfoData_id:              results.securityInfoData_id,
				})
			);
