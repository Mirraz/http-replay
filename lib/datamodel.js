...
DMResponse.prototype = {
	...
	saveSecurityInfo: function(obj, certs) {
	...
	saveSecurityInfoData: function(siData) {
		if ("parsed" in siData) {
			return Promise.all([
				this.dbConnPromise,
				this.saveSecurityInfo(siData.parsed.obj, siData.parsed.certs)
			])
			.then(
	...
	saveHttp: function(obj) {
		var httpChannelIdPromise =
			Promise.all([
				...
				(obj.securityInfo !== null ? this.saveSecurityInfoData(obj.securityInfo) : Promise.resolve(null) ),
			])
			.then( resArr => {
				return {
					dbConn:                           resArr[0],
