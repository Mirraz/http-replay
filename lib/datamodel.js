...
RDMRoot.prototype = {
	...
	getObservation: function(id) {
		if (this.isStopped) throw Error();
		let observation = new RDMObservation(this.dbConnPromise, id);
		this.obsPromises.push(observation.getOnDonePromise());
		return observation;
	},
...
function getObservationList() {
	...
	let obsListPromise = rdmRoot.getObservationList();
	rdmRoot.finish();
	return obsListPromise;
}
