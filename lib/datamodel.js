...
RDMRoot.prototype = {
	...
	getObservationRowDataList: function() {
		...
		let obsListPromise = this.dbConnPromise
			.then( dbConn => dbConn.execute('SELECT "id", "name" FROM "observations" ORDER BY "id"') )
			.then(
				rows => rows.map(
					row => ({
		...
		let rootPromise = this.dbConnPromise
			.then(
				dbConn => promiseWaitAll([].concat(this.obsPromises, this.obsListPromises))
					.finally( () => dbConn.close().then( () => {console.log("closed")/*XXX*/}) ),
...
RDMObservation.prototype = {
	...
	getResponseList: function() {
		if (this.isStopped) throw Error();
		let dbConnPromise = this.dbConnPromise;
		let respListPromise = this.dbConnPromise
			.then(
				dbConn => dbConn.executeCached(
						'SELECT ' +
						(
							[
								"http_channel_id",
						...
						' FROM "responses" WHERE "observation_id" = :observation_id',
						{observation_id: this.obsId}
					)
					.then(
						rows => rows.map(
							row => new RDMResponse(dbConn, {
								http_channel_id:       row.getResultByName("http_channel_id"),
								http_response_data_id: row.getResultByName("http_response_data_id"),
								http_status_id:        row.getResultByName("http_status_id"),
								cache_entry_id:        row.getResultByName("cache_entry_id"),
							})
						)
					)
			);
...
RDMResponse.prototype = {
	...
	getHttp: function() {
		...
		let httpChannelPromise = Promise.resolve(this.dbConn)
			.then(
				dbConn => dbConn.executeCached(
						'SELECT "uri" FROM "http_channels" WHERE "id" = :id',
						{id: this.respRowData.http_channel_id}
					)
			)
			.then( rows => {
				if (rows.length !== 1) throw Error("http_channels.rows.length != 1");
				return rows[0];
			})
			.then(
				row => ({
					uri: row.getResultByName("uri")
				})
			);
