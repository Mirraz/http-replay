...
function selectObjs(dbConn, selectSql, colNames, params) {
	return dbConn.execute(selectSql, params)
		.then(
			rows => rows.map( row => {
				let obj = {};
...
RDMResponse.prototype = {
	...
	getHttp: function() {
		...
		const sql = makeSelectSql("http_channels", colNames, 'WHERE "id" = :id');
		let httpChannelPromise = Promise.resolve(this.dbConn)
			.then( dbConn => selectObjs(dbConn, sql, colNames, {id: this.respRowData.http_channel_id}) )
			.then( objs => objs[0] );
