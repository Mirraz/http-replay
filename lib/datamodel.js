...
DMResponse.prototype = {
	...
	saveHttp: function(obj) {
			...
			if (securityInfoData === null) {
				...
				if (securityInfo === null) {
					...
					if (securityInfo.failedCertChain === null) {
						...
						certListOO = DMResponse.certListCallback.bind(undefined, securityInfo.failedCertChain.certList);
					}
					securityInfoOO = {
		...
		let responseIdPromise = this.responseIdPromise;
		let httpChannelOO = {
			"http_channels": {
				response_id: function(){return responseIdPromise},
				http_topic_id: {"http_topics": {value: obj.topic}},
				http_request_method_id: {"http_request_methods": {value: obj.request.method}},
				uri: obj.request.URI,
				http_request_header_list_id: DMResponse.httpRequestHeaderListCallback.bind(undefined, obj.request.headers),
				referrer: obj.request.referrer,
				statusCode: obj.response.statusCode,
				statusText: obj.response.statusText,
				http_response_header_list_id: DMResponse.httpResponseHeaderListCallback.bind(undefined, obj.response.headers),
		...
		// resolves with [httpChannelId, {<side promises results>}]
...
DMResponse.certListCallback = function(certList, executor) {
	let certListIdPromise = executor.execute({"certLists": {}});
	executor.addSidePromise(
		"certLists_to_certObjs",
		certListIdPromise
			.then(
				certListId => Promise.all(
					certList.map(
						certObj => executor.execute({
							"certLists_to_certObjs": {
	...
	return certListIdPromise;
};
DMResponse.httpRequestHeaderListCallback = function(headerList, executor) {
	let headerListIdPromise = executor.execute({"http_request_header_lists": {}});
	executor.addSidePromise(
		"http_request_header_lists_to_entries",
		headerListIdPromise
			.then(
				headerListId => Promise.all(
					headerList.map(
						header => executor.execute({
							"http_request_header_lists_to_entries": {
	...
	return headerListIdPromise;
};
DMResponse.httpResponseHeaderListCallback = function(headerList, executor) {
	let headerListIdPromise = executor.execute({"http_response_header_lists": {}});
	executor.addSidePromise(
		"http_response_header_lists_to_entries",
		headerListIdPromise
			.then(
