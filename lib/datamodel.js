...
function makeSelectSql(tableName, colNames, suffix) {
	return 'SELECT ' +
		colNames.map( colName => '"' + colName + '"' ).join(', ') +
		'FROM "' + tableName + '"' +
...
RDMRoot.prototype = {
	...
	getObservationRowDataList: function() {
		...
		const sql = makeSelectSql("observations", colNames, 'ORDER BY "id"');
...
RDMObservation.prototype = {
	...
	getResponseList: function() {
		...
		const sql = makeSelectSql("responses", colNames, 'WHERE "observation_id" = :observation_id');
...
RDMResponse.prototype = {
	...
	getHttp: function() {
		...
		const sql = '' +
			'SELECT ' + sqlColNamesStr + ' ' +
			'FROM "http_channels"' + ' ' +
			'JOIN "http_topics"' + ' ' +
				'ON "http_channels"."http_topic_id" = "http_topics"."id"' + ' ' +
			'JOIN "http_request_methods"' + ' ' +
				'ON "http_channels"."http_request_method_id" = "http_request_methods"."id"' + ' ' +
			'JOIN "http_response_content_types"' + ' ' +
				'ON "http_channels"."http_response_content_type_id" = "http_response_content_types"."id"' + ' ' +
			'JOIN "http_response_content_charsets"' + ' ' +
				'ON "http_channels"."http_response_content_charset_id" = "http_response_content_charsets"."id"' + ' ' +
			'WHERE "http_channels"."id" = :id';
		let httpChannelPromise = Promise.resolve(this.dbConn)
			.then( dbConn => selectObjs(dbConn, sql, resColNames, {id: this.respRowData.http_channel_id}) )
			.then( objs => objs[0] );
