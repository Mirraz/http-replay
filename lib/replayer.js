...
Replayer.playObservation = function(observationId) {
	try {
		...
		rdmObservation = undefined;
		
		respListPromise
			.then(
				respList => Promise.all(
					respList.map( rdmResp => {
						let httpPromise   = rdmResp.getHttp();
						let dataPromise   = rdmResp.getData();
						let statusPromise = rdmResp.getHttpStatus();
						let cachePromise  = rdmResp.getCacheEntry();
						rdmResp.finish();
						return Promise.all([httpPromise, dataPromise, statusPromise, cachePromise]);
					})
				)
			)
			.then( resArr => {
				resArr.forEach(
					results => {
						...
						let cache  = results[3];
						console.log("http.uri = " + http.request.URI);
						if (data === null) {
